...
之前的章节都是在本地机器上操作，但在实际开发中，往往会用一个公共的版本管理服务器，大家通过它分享代码，这个服务器经常被称为代码服务器。Git 本身能做到去中心化，但实际应用中，大多数时还是使用中心化方式。本章选用 GitHub 作为远程服务器，讲解如何和远程交互。如何搭建服务器将在后续章节讲解。

本章提到的很多内容，都和前面的章节有关系，但不是重复，前面章节回避了需要远程交互的内容，本章会一一补上。
...
可以在任意目录执行以下指令将远端(GitHub)上的仓库 clone 到本地：
...
还可以先在远端库创建好分支后，再同步到本地。具体可以登录到 GitHub 的[项目主页](https://github.com/iridiumcao/demo20231213)，然后创建远程分支，假如新创建的分支是 new-remote.

![](images/ch09_06_create_branch.png)

远端创建的分支，本地必须先 `fetch` 到本地，才能继续。

先到任意分支执行 `git fetch --all` 以下载所有远端的变化：
...
通过屏幕回显可知有远端一个新的分支 `origin/new-remote`，我们可以直接切换过去，就在本地创建了一个同名的分支，并且和远端自动关联起来了：
...
从上面看到，远端的分支 `new_branch_by_z3` 已经获取不到了。清理一下本地对远端库的引用：
...
如果一个分支，张三和李四都在修改，他们对同一文件的做的修改可能冲突，这种情况，应该先解决冲突，慎用 `git push --force` 指令，带了 `--force` 参数的 `push` 将用当前用户的改动覆盖掉服务器的，除非你确信要这么做，否则永远别用它。

【6. 小结】

本地和远端的交互主要通过这三个指令：`fetch` / `pull` / `push`

* `git fetch` 获取远端的变化，但不改变工作区的 HEAD，比如当前分支有更新的化，`pull` 执行结束后，HEAD 不会指向最新的 commit
...
* `git push origin develop` 推送到指定的远程分支上，如果这个分支是第一次推送，则在推送后，本地当前的分支将和远程同名的分支建立联系

## 7. 关联远程分支

本地库要通过远端库分享代码，就需要把本地分支和远端的分支关联起来。

我们可以通过在 `pull` 或 `push` 时可以自动关联分支。需要注意，`push` 时需要加参数 `--set-upstream` (可简作 `-u`) 才能建立关联。
...
本地库要通过远端库分享代码，首先要将本地库和远端库关联起来。通过 `git clone` 操作建立的新库会自动将本地库和它 `clone` 的远端库关联起来。如：
...
远程库，如果只有一个，约定俗成的名称是 `origin`，当然也可以起其他名称。

### 8.2 再谈关联分支

`git remote show origin` 分 `pull` 和 `push` 显示比 `git branch -vv` 更加详细的本地库和远端库的关联情况，例如
...
* `git branch -b {local branch name} -t origin/{remote branch name}` 将 pull 和 push 都与远程的分支关联起来了
  * 这条指令的 `-t` 标记可以省略：`git branch -b {local branch name} origin/{remote branch name}`
...
* `git fetch --all` 获取远程所有的变化
* `git fetch origin test2` 获取远程库 origin 的分支 test2 变化

执行上述指令后，如果当前分支在远端有新的变化，执行 `git status` 时屏幕会提示用户执行 `git pull` 去更新。
...
直观上看，`pull` 与 `fetch` 最大的不同是库更新后，它会调整 `HEAD` 到最新的变化上。
...
一个 commit 可能存在多个 branch 上，可以通过 `git branch --merged` 找到，例如
...
上面找到了所有包含 c540eeb 的分支。这个指令可以和远程指令结合起来用，如果不跟 commit id, 就表示查本分支包含哪些分支的全部内容，例如：
...
它是说在远端库中，这三个分支都已经合并到 master 了：`helloworld001`, `helloworld002` and `helloworld003`.
