...
- [第九章 远程交互](#第九章-远程交互)
  ...
  - [8. 进阶篇](#8-进阶篇)
    ...
    - [8.3 `git checkout -b` VS `git checkout -B`](#83-git-checkout--b-vs-git-checkout--b)
    - [8.4 再谈 fetch/pull](#84-再谈-fetchpull)
      - [8.4.1 `fetch`](#841-fetch)
      - [8.4.2 `pull`](#842-pull)
    - [8.5 批量处理多个分支](#85-批量处理多个分支)
    - [8.6 `git branch --merged`](#86-git-branch---merged)
    - [8.7 Git 网络拓扑结构](#87-git-网络拓扑结构)
...
`-v` 能查看到更详细的分支信息：分支名，最后一个 commit 的 id 和 message：
...
```PowerShell
PS D:\iridiumcao.github.io> git branch -vv
  202104                             7773573 [origin/202104] 2021.04
  blockchain-refine                  5197b2e fix: correct minor text formatting in Bitcoin book recommendations
  english                            e118a3f [origin/english: gone] english, NCE 3
  find-and-replace                   2e7b39c [origin/find-and-replace: ahead 3, behind 2] Add find and replace guide
```

这个例子中能看到的信息比较多：

- `202104` 和远端的完全一样
- `blockchain-refine` 仅仅是一个本地分支
- `english` 曾经有一个对应的远程分支，现在远端的已经删了，只剩下本地分支
- `find-and-replace` 本地和远端的分支的内容出现差异 (ahead 3, behind 2)
...
### 8.3 `git checkout -b` VS `git checkout -B`

- `git checkout -b` 用来新建一个本地分支，上面已经有讲。
- `git checkout -B` 用来创建一个本地分支，或者把一个已有的本地分支强制重置到指定的起点上。

比如

```bash
git checkout -B hello_branch origin/hello_branch
```

表示：

- 如果本地没有分支 `hello_branch`，则创建本地分支 `hello_branch`，并让它从 `origin/hello_branch` 出发
- 如果本地已经有分支 `hello_branch`，则把它强制重置到 `origin/hello_branch` 当前指向的 commit 上

这个例子里之所以会和远程分支「对齐」，是因为这里指定的起点刚好是 `origin/hello_branch`；如果把起点换成别的 ref，例如 `master` 或 `HEAD~3`，`git checkout -B` 同样成立。
