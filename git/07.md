...
[⇦上一章](06.md) - [首页🏠](index_zh.md) - [下一章⇨](08.md)
  ...
  - [8. 进阶篇](#8-进阶篇)
    ...
    - [8.4 修改 commit 的元信息](#84-修改-commit-的元信息)
      - [8.4.1 `git commit --amend`](#841-git-commit---amend)
        ...
        - [8.4.1.3 将新的改动附加到最后一个 commit 中](#8413-将新的改动附加到最后一个-commit-中)
...
（更详细的内容参[Git Commit 中的时间记录](date_in_commit.md)）
...
##### 8.4.1.3 将新的改动附加到最后一个 commit 中

有的时候，我们完成提交后，又要改一些文件，改完之后希望把这这些改动合并到上一个 commit 里，此时，我们可以这样分步完成：

1. 做一次新的提交
2. 把新的提交和前一次合并

上面的操作当然可行，但比较繁琐，更简单的办法是：

1. 用 `git add` 把新的改动提交到暂存区
2. 执行 `git commit --amend --no-edit` 即可把新的改动合并到最后一个 commit 里

（这个方便法门是 Charles 告诉我的）
