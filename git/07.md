...
- [第七章 修改历史](#第七章-修改历史)
  - [0. 实验准备](#0-实验准备)
  - [1. 重置版本 `reset`](#1-重置版本-reset)
    - [1.1 实验，重置最近的一个版本](#11-实验重置最近的一个版本)
    - [1.2 实验，重置最近的多个版本](#12-实验重置最近的多个版本)
    - [1.3 硬重置(hard reset)](#13-硬重置hard-reset)
    - [1.4 综合例子](#14-综合例子)
  - [2. 回滚某个版本 revert](#2-回滚某个版本-revert)
  - [3. 删除记录](#3-删除记录)
  - [4. 合并记录](#4-合并记录)
  - [5. 修改记录](#5-修改记录)
  - [6. 修改 message](#6-修改-message)
  - [7. 进阶篇](#7-进阶篇)
    - [7.1 实验——删除记录](#71-实验删除记录)
    - [7.2 实验——通过 `reflog` 找回删除的版本](#72-实验通过-reflog-找回删除的版本)
    - [7.3 调整版本的顺序](#73-调整版本的顺序)
    - [7.4 修改 commit 的元信息](#74-修改-commit-的元信息)
      - [7.4.1 `git commit --amend`](#741-git-commit---amend)
        - [7.4.1.1 修改 commit 的作者](#7411-修改-commit-的作者)
        - [7.4.1.2 修改 commit 的时间](#7412-修改-commit-的时间)
      - [7.4.2 修改任意 commit 元信息的方法](#742-修改任意-commit-元信息的方法)
    - [7.5 拆分 commit](#75-拆分-commit)
    - [7.6 HEAD~n VS HEAD@{n}](#76-headn-vs-headn)

---

[上一章](06.md)讲的是如何记录文档的版本变化——用 commit，一个 commit 对应一个文档版本。多个 commit 串起来，就成了历史记录，可以通过 `git log` 查看。
...
`git reset HEAD~n` 可以理解为撤销 n 个版本，也可以理解为撤销到版本 `HEAD~n`.

### 1.3 硬重置(hard reset)

硬重置也是重置，但和前面的区别是：

* 普通重置：重置后，被撤销的版本会回滚到工作区。
...
这条命令执行后，通过 `git status` 会发现工作区是没东西的。
...
* 指定重置到 aeaefe37 版本，假若刚才删的就是这个版本，该命令相当于取消删除了。
  * `git reset aeaefe37`
...
当代码提交后，发现消息需要修改。我们可以直接 reset 这个消息，重新提交，顺便改掉 message. 但也可以通过命令修改。
...
初学者可以花几分钟时间，认真读完交互界面中的文字提示。注意我用红线框出的部分，它提示删除 commit 的指令是 `pick`，简写 `p`，因此把要删除的第二行的 `pick` 改成 `drop` 再保存退出交互界面。
...
V3 已经消失，V4 和 V5 的版本ID(commit id)发生了变化，ID 的生成算法，一来 pareent ID，parent ID 变化了，它也就变了。

![](images/ch07_07_git_rebase_drop.png)

### 7.2 实验——通过 `reflog` 找回删除的版本

reflog, refrence log, 记录了所有改变 repo 的历史。

重置之后，原先的版本并未消失，还可以复原。通过 `git reflog` 找到记录即可。下面是完整的实验过程：
...
#### 7.4.1 `git commit --amend`

`git commit --amend` 主要用于改写最后一个 commit 的元信息。

##### 7.4.1.1 修改 commit 的作者
