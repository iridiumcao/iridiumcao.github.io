...
如果你对刚刚提交的版本不满意，想要收回，继续改了再提交，怎么处理？假如你在文档或代码里写了脏话，并且提交了，然后你返回了，怎么办？Git 已经考虑到了，重置刚才的操作即可。
...
前面提到可以通过 `git commit --amend` 修改最近的版本的元信息。修改任意位置的 commit 元信息，和上段的思路一样，也是把需要修改的 commit 通过创建新分支的方法放在最后一个，改好后再 cherry-pick 原本在它之后的 commit.
