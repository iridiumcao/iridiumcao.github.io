...
([English](Recommended_Usage_of_Git.md))
...
每日开始工作的第一件事，集成 master branch 的最新代码。先更新 master 分支，再通过 rebase 更新工作分支。
...
在你执行合并(merge)或变基(rebase)操作之前，可以使用 `git branch bak_bransh` 创建一个备份分支，以备用。
...
注：本文最早大约是2016年初写给公司同事参考使用，考虑到无敏感性而有通用性，搬到本站发布。
