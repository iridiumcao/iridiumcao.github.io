...
- [第十三章 创建和使用 Git 服务节点](#第十三章-创建和使用-git-服务节点)
  ...
  - [4. 本地仓库](#4-本地仓库)
  - [5. 设置远端仓库](#5-设置远端仓库)
    - [5.1 查看并添加远端库](#51-查看并添加远端库)
    - [5.2 更新远端库 URL](#52-更新远端库-url)
    - [5.3 解除远端库关联](#53-解除远端库关联)
    - [5.4 多远端库下的本地库操作](#54-多远端库下的本地库操作)
...
使用 `--bare` 参数创建仓库，服务器一般无需工作区(Working Directory)，裸库一般以 `.git` 作为后缀命名
...
客户端节点对操作系统无所谓，但也需要到官网上下载 git 安装，可参[第三章](03.md)。注意，git 是去中心化的结构，每个节点都是平等的，并没有所谓的服务端和客户端的区分。我们这里区分服务端和客户端只是人为地在业务上的主观划分而已，所以不论服务端和客户端，它们安装的 git 软件都是一样的。

Git 安装好后，也要进行一些必要的设置，最最基本的设置，就是使用者的姓名和邮箱
...
`ssh-keygen` 生成 ssh key，已经生成过可以忽略，`ssh-copy-id` 将前面生成的 SSH 公钥传送给服务器。这两个指令输入并回车后，有几处需要确认，直接按回车即可（实验环境，便宜行事）。
...
安装后，最好创建一个脚本 `/home/caoyi/sandbox/syn_mirror.sh`，定期更新。
...
多数情况下，我们 clone 好本地仓库之后，接着用就行，本地库和远端库的关联是自动建好的，无需多考虑。但是有些场景下，我们不得不去维护远端库关联。
...
* 将同一个项目推送到多个远端仓库时，比如我们把项目同时发布到 GitHub 和 Gitlab
* We have two IP of a same repository and the connections are not stable

### 5.1 查看并添加远端库

我们通过 `git clone` 创建本地仓库后，本地仓库就自然和远端的仓库建立了联系，可以通过 `git remote` 指令查看，如：
...
* `git remote` 查看远端仓库，一般默认的就是 origin
* `git remote -v` 查看远端仓库的详细信息

Usually, a local git repository tracks a default remote repository named _origin_. However, we also needs to work with multiple remote repositories in some cases. 前面讲述镜像库时，就设置了两个远端仓库。

添加远端库使用的指令是 `git remote add {remote repo name} {remote repo URL}`, 这里的 `{remote repo name}` 是自定义的。
...
如果想要解除和某个库的关联，可以使用 `git remote rm`，如
...
如果一个本地库关联了多个远端库，则它在执行 `pull` 或 `push` 等指令时，不能省略库名，比如
...
从远端拉取分支时，也要明确指明从哪个仓库拉取 (`git switch/checkout --track {remote repo name}/{branch name}`)：
...
如果直接执行 `git switch branch_hello` 会失败，因为 git 不知道从哪个远端库中取。

### 5.5 `git remote` 指令总结

* `git remote` 简明显示当前库关联的远端库，只显示名称
* `git remote -v` 详细显示当前库关联的远端库，除了名称，还显示 git URL
* `git remote add {remote repo name} {git URL}` 给当前库增加一个关联的远端库
* `git remote rm {remote repo name}`将本地项目和某个远端仓库解除关联
* `git remote prune origin` 同步远端分支信息到本地，远端已经删除分支，本地的引用也会删除。
