# Git

[**English** \| [中文](index_zh.md)]

[Index](../index.md)

- [The Way to Keep Git Branches Clean](keeping_branches_clean.md)
- [How to Create a New Repository](init_repo.md)
- [How to Retrieve the Email of a Commit's Author](retrieve_commit_author_email.md)
- [How to check if a git branch exists in the local/remote repository?](verify_existence_git_branch.md)
- [Automating the Removal of Outdated and Merged Branches](clean_outdated_merged_branches.md)
- [Clean Files with a Long Path (Git)](clean_files_with_long_path.md)
- [Synchronizing a Remote Repository to Local Using Python](sync_remote_with_python.md)
- [Update Git Branch/Tag Naming Conventions](hook_branch_tag_naming_conventions.md)
- [Work with Multiple Remote Repositories](work_with_multi_remote_repositories.md)
- [Git Submodule Test](git_submodule_test.md)
- `git filter-repo`
  - [Move Files Between Git Repositories While Preserving History](move_files_with_history.md)
  - [How to Remove Large Files from Git History](remove_large_files.md)
- [Finding Commits Before a Merge Point in Git](finding_commits_before_a_merge_point.md)

<script src="https://giscus.app/client.js"
        data-repo="iridiumcao/iridiumcao.github.io"
        data-repo-id="MDEwOlJlcG9zaXRvcnkyOTUwNTIyODQ="
        data-category="Announcements"
        data-category-id="DIC_kwDOEZYj_M4Cxfqj"
        data-mapping="pathname"
        data-strict="0"
        data-reactions-enabled="1"
        data-emit-metadata="0"
        data-input-position="bottom"
        data-theme="preferred_color_scheme"
        data-lang="en"
        crossorigin="anonymous"
        async>
</script>