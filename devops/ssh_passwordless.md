...
[Index](ssh_passwordless.md)

---

- [SSH 免密码登录设置](#ssh-免密码登录设置)
  - [1. 安装软件](#1-安装软件)
  - [2. 生成密钥](#2-生成密钥)
  - [3. 添加认证](#3-添加认证)
    - [3.1 自动方式](#31-自动方式)
...
如果经常需要从 A 登录到 B，每次都输入密码就显得比较麻烦，可以考虑将 A 系统 `alice` 用户的 SSH 公钥添加到 B 上的用户 `bob` 的信任列表（即 `authorized_keys` 文件）中，实现免密码登录。具体步骤如下：
...
SSH 对权限非常敏感，权限不对 SSH 可能无法使用免密码登录，`.ssh` 目录应为 `700`，`authorized_keys` 应为 `600`，可以通过下面的指令设置：
